        ...
        for SCHEDULING in FOR STATIC DYNAMIC GUIDED; do
            if [ "$SCHEDULING" != "FOR" ]; then
                for CSIZE in 1 10 100 1000; do
                    gcc -o main main_old.c helpers.c -fopenmp -std=c99 -O3 -DTHREAD_NUMBER=$THREADS -D$SCHEDULING -DCHUNKSIZE=$CSIZE 
                    echo "Running with $THREADS threads and $SCHEDULING scheduling (chunk size: $CSIZE) on matrix $MATRIX"
                    ./main $MATRIX
                done
                continue
            fi            
            gcc -o main main_old.c helpers.c -fopenmp -std=c99 -O3 -DTHREAD_NUMBER=$THREADS -D$SCHEDULING
